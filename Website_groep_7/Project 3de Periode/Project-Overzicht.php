...
if (isset($_POST['PDF'])) {
    ...
    if ($_SESSION['gebruikersnaam'] == 'Admin') {
        $sql = "SELECT * FROM urenreg";
    } else
    {
    $sql = "SELECT * FROM urenreg WHERE OV_Nummer = $ov_nummer2";
    }
    ...
    if (isset($_POST['projectnaam']) && !empty($_POST['projectnaam'])) {
        ...
        if ($_SESSION['gebruikersnaam'] == 'Admin') {
            $sql = "SELECT * FROM urenreg WHERE Projectnaam = '$filter'";
        } else
        {
            $sql = "SELECT * FROM urenreg WHERE OV_Nummer = $ov_nummer2 AND Projectnaam = '$filter'";
        }
    }
...
<a href="Project-OverzichtTotaal.php"><button class="knop1">Totaal uren</button></a>
